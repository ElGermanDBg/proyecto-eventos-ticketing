...
## 3. CORS Restringido (Control de Acceso por Origen)
La API implementa una configuración de CORS (Cross-Origin Resource Sharing) restrictiva en lugar de aceptar peticiones de cualquier origen:
- **Origen específico:** Solo se permiten peticiones desde el dominio del frontend desplegado (`FRONTEND_URL`), bloqueando peticiones desde dominios no autorizados.
- **Métodos permitidos:** Se limitan a `GET`, `POST`, `PUT`, `DELETE` y `OPTIONS`.
- **Headers permitidos:** Solo se aceptan `Content-Type` y `Authorization`.
- Esto previene ataques de **CSRF (Cross-Site Request Forgery)** y acceso no autorizado a la API desde sitios maliciosos.
...
## 5. Protección de Credenciales (Bcrypt)
...
## 6. Autenticación y Autorización sin Estado (JWT)
...
## Conclusión
La combinación de estas 6 herramientas cubre las vulnerabilidades más críticas (Top 10 de OWASP) como inyecciones, fallas en control de acceso, problemas en la autenticación y configuración incorrecta de seguridad, creando un entorno seguro tanto para el cliente como para la integridad de los datos de la base de datos.
